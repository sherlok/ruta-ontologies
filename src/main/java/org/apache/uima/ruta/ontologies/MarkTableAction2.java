...
public class MarkTableAction2 extends AbstractRutaAction {
    ...
    public MarkTableAction2(ITypeExpression typeExpr, INumberExpression indexExpr,
    ...
    @Override
    public void execute(final MatchContext matchContext, final RutaStream rutaStream, final InferenceCrowd
            inferenceCrowd) {
        ...
        for (IStringExpression each : featureMap.keySet()) {
            map.put(each.getStringValue(matchContext, rutaStream), featureMap.get(each)
                    .getIntegerValue(matchContext, rutaStream));
        }
        ...
        int maxIgnoreCharValue = maxIgnoreChar != null ? maxIgnoreChar.getIntegerValue(matchContext, rutaStream) : 0;
